...
const {saveFileInfer} = require("./helpers/file");
  ...
  _io.on("connection", (socket) => {
    ...
    socket.on("start_train_model", async (data) => {
      ...
      await _io.emit(`start_training`, {
        data_dir: `${datasetData.savePath}/train`,
    ...
    socket.on("start_test_model", async (data) => {
      ...
      await _io.emit(`start_testing`, {
        test_data_dir: `${datasetData.savePath}/test`, 
        model_type: modelData.modelName,
        labId: data.labId,
        batch_size: config.batch_size, 
        testId: testId
    ...
    socket.on("start_infer_model", async (data) => {
      ...
      image_name_temp = uuid();
      let savePathFile = `.${savePath}/${image_name_temp}.png`;
      saveFileInfer({filename: savePathFile, file: data.file});
      
      await _io.emit(`start_infering`, {
        model_type: modelData.modelName,
        labId: data.labId,
        image_path: savePathFile,
        batch_size: config.batch_size,
        inferId: inferId
      });
